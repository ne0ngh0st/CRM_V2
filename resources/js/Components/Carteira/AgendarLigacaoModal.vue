...
function salvar() {
    form.post(route('carteira.agendamento', props.cliente.id), {
        preserveScroll: true,
        onSuccess: () => fechar(),
    });
}
...
<template>
    <Modal :show="show" max-width="md" @close="fechar">
        <form v-if="cliente" class="p-6" @submit.prevent="salvar">
            ...
            <div class="mt-4">
                <InputLabel for="data_agendamento" value="Data e hora *" />
                <TextInput
                    id="data_agendamento"
            ...
            <div class="mt-4">
                <InputLabel for="observacao_agendamento" value="Observação" />
                <textarea
