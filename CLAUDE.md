...
- **"Criar orçamento"** navega pra `/orcamentos` com query params e pré-preenche o modal.
- **Hero de aderência**: `CarteiraSegmentoCard` (mesmo da Home) no topo da página `/carteira`, abaixo do PageHero.

## Pendências
- Construir a página completa de Pedidos em Aberto (Carteira e Orçamentos já têm página própria) — reusar o Design System de Dashboard (seção acima).
