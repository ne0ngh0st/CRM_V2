...
| Como | Arquivo | Estado |
|---|---|---|
| **Duplo clique** | `infra/Enviar relatorios TOTVS.cmd` | ✅ **é o que se usa hoje** |
| Terminal | `infra/enviar-relatorios-totvs.sh` (Git Bash) | ✅ |
| Laço em segundo plano | `infra/vigiar-relatorios-totvs.ps1` | ✅ quando lançado à mão |
| ~~Auto-start no logon~~ | `infra/instalar-inicializacao.ps1` | ❌ **não dispara nesta máquina** |
| ~~Tarefa agendada~~ | `infra/instalar-tarefa-upload.ps1` | ❌ **reporta sucesso sem executar** |

#### 🔴 Nenhuma automação de LOGON funciona nesta máquina

Duas tentativas, dois modos de falha diferentes, ambos silenciosos (2026-09-05):

**1. Agendador de Tarefas — reporta sucesso sem executar.** Aceita registrar, dispara no
horário, cria o processo e grava no log de eventos "concluída com sucesso, código de
retorno 0" — e a ação não roda. Reduzido ao caso mínimo antes de concluir: uma tarefa
rodando `C:\Windows\System32\cmd.exe /c echo ok > arquivo.txt` no próprio perfil do
usuário **não cria o arquivo**, e ainda assim reporta 0. Não é o script, não é permissão de
registro (registrar funciona), não é caminho: é política da máquina.

**2. Pasta de Inicialização — o item não é executado no logon.** O `.vbs` foi instalado, o
caminho está correto (sem redirecionamento de pasta), não está desabilitado em
`StartupApproved`, e **funciona quando chamado à mão**. Depois de um reboot real, 5,5
minutos depois do boot: nenhum processo, log sem uma linha. Suspeita não confirmada: `.vbs`
na Inicialização é a técnica clássica de persistência de malware e EDR corporativo costuma
bloquear exatamente isso no logon, permitindo a execução manual.

⚠️ **A lição vale mais que a feature: `LastTaskResult = 0` não é prova de que algo
aconteceu, e "o arquivo está na pasta de Inicialização" também não.** A prova é o efeito —
o log crescer. Nos dois casos, parar no sinal aparente teria entregue uma automação morta,
e o sintoma apareceria semanas depois como "o CRM está com dado velho" — exatamente o mês
de sincronização parada que a §10 descreve.

Por isso `instalar-tarefa-upload.ps1` **se auto-verifica**: dispara a tarefa, espera o log
crescer e, se não crescer, **remove a tarefa** e avisa. Tarefa que mente é pior que nenhuma.

**O que sobrou, e é suficiente:** o Tony dá um duplo clique em
`infra/Enviar relatorios TOTVS.cmd` depois de gerar os relatórios. Daí para frente é tudo
automático — o cron da app-2 importa de hora em hora. Quem quiser deixar subindo sozinho
durante o expediente pode lançar `vigiar-relatorios-totvs.ps1` à mão uma vez: ele funciona,
o que não funciona é o Windows lançá-lo por conta própria.

Os dois instaladores continuam no repositório porque **funcionam em máquina sem essa
política** — e porque o diagnóstico acima é o que impede alguém de tentar de novo do zero.
Dois defeitos reais de API foram corrigidos neles no caminho:
